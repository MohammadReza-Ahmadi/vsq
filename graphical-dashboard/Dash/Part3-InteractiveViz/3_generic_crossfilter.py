...
app.layout = html.Div([
    # the main application div with 3 figures, (dcc.Graph)
    # four columns is used i guess cuz figures are bigger than typical columns.
    # so we have one row and some columns.
    html.Div(
...
# the figures have a predefined attr: selectedData which will be used here.
@app.callback(
    Output('g1', 'figure'),
    Output('g2', 'figure'),
    Output('g3', 'figure'),
...
def callback(selection1, selection2, selection3):
    # use the intersection of all the selections to create the figures.
    selectedpoints = df.index
    # go over the selected points.
    print(selectedpoints)
    print(selection1)
    for selected_data in [selection1, selection2, selection3]:
        # if there are points regarding that selected part (samples in the box).
