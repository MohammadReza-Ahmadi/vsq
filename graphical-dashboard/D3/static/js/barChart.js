...
// looks like a on click function that using the updated groups returns the new dataset
function datasetBarChosen(group, barData) {
        var ds = [];
        for (x in barData) {
             if(barData[x].group==group){
                 ds.push(barData[x]);
             }
            }
        return ds;
...
function d3BarChart(barData) {
	var firstDatasetBarChart = datasetBarChosen(group, barData);
