...
from app.core.settings import initial_score_change_store_config, NUMBER_OF_DECIMAL_PLACES
...
class ScoreCalculationService:
    ...
    def calculate_user_final_score(self, recent_p: Profile = None, revised_p: Profile = None, reset_cache=False):
        total_pure_score = self.calculate_user_profile_score(revised_p=revised_p)
        total_pure_score += self.calculate_user_done_trades_score(revised_p=revised_p)
        total_pure_score += self.calculate_user_undone_trades_score(revised_p=revised_p)
        total_pure_score += self.calculate_user_loans_score(revised_p=revised_p)
        total_pure_score += self.calculate_user_cheques_score(revised_p=revised_p)
    ...
    def calculate_user_profile_score(self, recent_p: Profile = None, revised_p: Profile = None, reset_cache=False):
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_identities_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.address_verification_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.identities_score += score_change
        # round identities_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.identities_score = round(revised_p.identities_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.address_verification_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[address_verification-i4]= {}'.format(normalized_revised_rule_score, revised_p.address_verification))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_identities_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.has_kyc_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.identities_score += score_change
        # round identities_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.identities_score = round(revised_p.identities_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.has_kyc_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[has_kyc-i1]= {}'.format(normalized_revised_rule_score, revised_p.has_kyc))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.membership_date_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.membership_date_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[membership_date-h5]= {}, profile_member_ship_days_count={}'.format(normalized_revised_rule_score, revised_p.membership_date, member_ship_days_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_identities_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.military_service_status_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.identities_score += score_change
        # round identities_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.identities_score = round(revised_p.identities_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.military_service_status_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[military_service_status-i2]= {}'.format(normalized_revised_rule_score, revised_p.military_service_status))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.recommended_to_others_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.recommended_to_others_count_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[recommended_to_others_count-h8]= {}'.format(normalized_revised_rule_score, get_zero_if_none(revised_p.recommended_to_others_count)))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_identities_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.sim_card_ownership_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.identities_score += score_change
        # round identities_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.identities_score = round(revised_p.identities_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.sim_card_ownership_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[sim_card_ownership-i3]= {}'.format(normalized_revised_rule_score, revised_p.sim_card_ownership))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_p.star_count_average_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_p.star_count_average_score = normalized_revised_rule_score
        profile_score += normalized_revised_rule_score
        profile_score = round(profile_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, profile:[star_count_average-h9]= {}'.format(normalized_revised_rule_score, revised_p.star_count_average))
    ...
    def calculate_user_done_trades_score(self, revised_p: Profile = None, reset_cache=False, recent_dt: DoneTrade = None, revised_dt: DoneTrade = None):
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.timely_trades_count_of_last_3_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.timely_trades_count_of_last_3_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[timely_trades_count_of_last_3_months_h6]= {}'.format(normalized_revised_rule_score, revised_dt.timely_trades_count_of_last_3_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.timely_trades_count_between_last_3_to_12_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.timely_trades_count_between_last_3_to_12_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[timely_trades_count_between_last_3_to_12_months-h7]= {}'.format(normalized_revised_rule_score, revised_dt.timely_trades_count_between_last_3_to_12_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.past_due_trades_count_of_last_3_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.past_due_trades_count_of_last_3_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[past_due_trades_count_of_last_3_months-t22]= {}'.format(normalized_revised_rule_score, revised_dt.past_due_trades_count_of_last_3_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.past_due_trades_count_between_last_3_to_12_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.past_due_trades_count_between_last_3_to_12_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[past_due_trades_count_between_last_3_to_12_months-t23]= {}'.format(normalized_revised_rule_score, revised_dt.past_due_trades_count_between_last_3_to_12_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.arrear_trades_count_of_last_3_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.arrear_trades_count_of_last_3_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[arrear_trades_count_of_last_3_months-t24]= {}'.format(normalized_revised_rule_score, revised_dt.arrear_trades_count_of_last_3_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.arrear_trades_count_between_last_3_to_12_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.arrear_trades_count_between_last_3_to_12_months_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[arrear_trades_count_between_last_3_to_12_months-t25]= {}'.format(normalized_revised_rule_score, revised_dt.arrear_trades_count_between_last_3_to_12_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.average_total_balance_ratios_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.average_total_balance_ratios_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[avg_total_balance-v12]= {}'.format(normalized_revised_rule_score, avg_total_balance_ratio))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_dt.average_delay_days_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_dt.average_delay_days_score = normalized_revised_rule_score
        done_trades_score += normalized_revised_rule_score
        # round done_trades_score upto NUMBER_OF_DECIMAL_PLACES
        done_trades_score = round(done_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, doneTrades:[avg_delay_days-t28]= {}'.format(normalized_revised_rule_score, revised_dt.total_delay_days))
    ...
    def calculate_user_undone_trades_score(self, revised_p: Profile = None, reset_cache=False, recent_udt: UndoneTrade = None, revised_udt: UndoneTrade = None, dt: DoneTrade = None):
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.undue_trades_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.undue_trades_count_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[undue_trades_count-h10]= {}'.format(normalized_revised_rule_score, revised_udt.undue_trades_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.undue_trades_total_balance_of_last_year_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.undue_trades_total_balance_of_last_year_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[undue_total_balance_ratio-v15]= {}'.format(normalized_revised_rule_score, undue_total_balance_ratio))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.past_due_trades_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.past_due_trades_count_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[past_due_trades_count-t26]= {}'.format(normalized_revised_rule_score, revised_udt.past_due_trades_count))
        ...
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        # handle score change reason
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.past_due_trades_total_balance_of_last_year_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.past_due_trades_total_balance_of_last_year_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[past_due_total_balance_ratio-v13]= {}'.format(normalized_revised_rule_score, past_due_total_balance_ratio))
        ...
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        # handle score change reason
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.arrear_trades_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.arrear_trades_count_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[arrear_trades_count-t27]= {}'.format(normalized_revised_rule_score, revised_udt.arrear_trades_count))
        ...
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        # handle score change reason
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_udt.arrear_trades_total_balance_of_last_year_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_udt.arrear_trades_total_balance_of_last_year_score = normalized_revised_rule_score
        undone_trades_score += normalized_revised_rule_score
        # round undone_trades_score upto NUMBER_OF_DECIMAL_PLACES
        undone_trades_score = round(undone_trades_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, undoneTrades:[arrear_total_balance_ratio-v14]= {}'.format(normalized_revised_rule_score, arrear_total_balance_ratio))
    ...
    def calculate_user_loans_score(self, revised_p: Profile = None, reset_cache=False, recent_ln: Loan = None, revised_ln: Loan = None):
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_histories_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.loans_total_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.histories_score += score_change
        # round histories_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.histories_score = round(revised_p.histories_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.loans_total_count_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[loans_total_count-h11]= {}'.format(normalized_revised_rule_score, revised_ln.loans_total_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.monthly_installments_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.monthly_installments_total_balance_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[installments_total_balance_ratio-v16]= {}'.format(normalized_revised_rule_score, installments_total_balance_ratio))

        # should be calculate user_total_loans_balance
        revised_ln.loans_total_balance = ZERO if revised_ln.loans_total_balance is None else revised_ln.loans_total_balance
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.overdue_loans_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.overdue_loans_total_balance_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[overdue_total_balance_ratio-v18]= {}'.format(normalized_revised_rule_score, overdue_total_balance_ratio))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.past_due_loans_total_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.past_due_loans_total_count_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[past_due_loans_total_count-t33]= {}'.format(normalized_revised_rule_score, revised_ln.past_due_loans_total_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.past_due_loans_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.past_due_loans_total_balance_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[past_due_total_balance_ratio-v19]= {}'.format(normalized_revised_rule_score, past_due_total_balance_ratio))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.arrear_loans_total_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.arrear_loans_total_count_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[arrear_loans_total_count-t34]= {}'.format(normalized_revised_rule_score, revised_ln.arrear_loans_total_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.arrear_loans_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.arrear_loans_total_balance_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[arrear_total_balance_ratio-v20]= {}'.format(normalized_revised_rule_score, arrear_total_balance_ratio))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.suspicious_loans_total_count_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.suspicious_loans_total_count_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[suspicious_loans_total_count-t35]= {}'.format(normalized_revised_rule_score, revised_ln.suspicious_loans_total_count))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ln.suspicious_loans_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ln.suspicious_loans_total_balance_score = normalized_revised_rule_score
        loans_score += normalized_revised_rule_score
        # round loans_score upto NUMBER_OF_DECIMAL_PLACES
        loans_score = round(loans_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, loans:[suspicious_total_balance_ratio-v21]= {}'.format(normalized_revised_rule_score, suspicious_total_balance_ratio))
    ...
    def calculate_user_cheques_score(self, revised_p: Profile = None, reset_cache=False, recent_ch: Cheque = None, revised_ch: Cheque = None):
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ch.unfixed_returned_cheques_count_between_last_3_to_12_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ch.unfixed_returned_cheques_count_between_last_3_to_12_months_score = normalized_revised_rule_score
        cheques_score += normalized_revised_rule_score
        # round cheques_score upto NUMBER_OF_DECIMAL_PLACES
        cheques_score = round(cheques_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, cheques:[unfixed_returned_cheques_count_between_last_3_to_12_months-t30]= {}'.format(normalized_revised_rule_score, revised_ch.unfixed_returned_cheques_count_between_last_3_to_12_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ch.unfixed_returned_cheques_count_of_last_3_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ch.unfixed_returned_cheques_count_of_last_3_months_score = normalized_revised_rule_score
        cheques_score += normalized_revised_rule_score
        # round cheques_score upto NUMBER_OF_DECIMAL_PLACES
        cheques_score = round(cheques_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, cheques:[unfixed_returned_cheques_count_of_last_3_months-t29]= {}'.format(normalized_revised_rule_score, revised_ch.unfixed_returned_cheques_count_of_last_3_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ch.unfixed_returned_cheques_count_of_last_5_years_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ch.unfixed_returned_cheques_count_of_last_5_years_score = normalized_revised_rule_score
        cheques_score += normalized_revised_rule_score
        # round cheques_score upto NUMBER_OF_DECIMAL_PLACES
        cheques_score = round(cheques_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, cheques:[unfixed_returned_cheques_count_of_last_5_years-t32]= {}'.format(normalized_revised_rule_score, revised_ch.unfixed_returned_cheques_count_of_last_5_years))

        normalized_revised_rule_score = rds.get_score_of_rules_unfixed_returned_cheques_count_of_more_12_months_t31(get_zero_if_none(revised_ch.unfixed_returned_cheques_count_of_more_12_months))
        # handle score change reason
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_timeliness_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ch.unfixed_returned_cheques_count_of_more_12_months_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.timeliness_score += score_change
        # round timeliness_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.timeliness_score = round(revised_p.timeliness_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ch.unfixed_returned_cheques_count_of_more_12_months_score = normalized_revised_rule_score
        cheques_score += normalized_revised_rule_score
        # round cheques_score upto NUMBER_OF_DECIMAL_PLACES
        cheques_score = round(cheques_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, cheques:[unfixed_returned_cheques_count_of_more_12_months-t31]= {}'.format(normalized_revised_rule_score, revised_ch.unfixed_returned_cheques_count_of_more_12_months))
        ...
        # calculate normalized identities score
        normalized_revised_rule_score = self.calculate_volumes_normalized_score(revised_rule_score)
        score_change = self.resolve_and_save_score_change(revised_p, rule_code, recent_ch.unfixed_returned_cheques_total_balance_score, normalized_revised_rule_score)
        # update score values in related entity
        revised_p.volumes_score += score_change
        # round volumes_score upto NUMBER_OF_DECIMAL_PLACES
        revised_p.volumes_score = round(revised_p.volumes_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        revised_ch.unfixed_returned_cheques_total_balance_score = normalized_revised_rule_score
        cheques_score += normalized_revised_rule_score
        # round cheques_score upto NUMBER_OF_DECIMAL_PLACES
        cheques_score = round(cheques_score, NUMBER_OF_DECIMAL_PLACES) if NUMBER_OF_DECIMAL_PLACES >= ZERO else None
        print('score= {}, cheques:[total_balance_ratio-v17]= {}'.format(normalized_revised_rule_score, total_balance_ratio))
    ...
    # noinspection PyMethodMayBeStatic
    def calculate_profile_rounded_score(self, revised_p: Profile):
        revised_p.identities_score = round(revised_p.identities_score)
        revised_p.histories_score = round(revised_p.histories_score)
        revised_p.volumes_score = round(revised_p.volumes_score)
        revised_p.timeliness_score = round(revised_p.timeliness_score)
        revised_p.score = revised_p.identities_score + revised_p.histories_score + revised_p.volumes_score + revised_p.timeliness_score
    ...
    def calculate_identities_normalized_score(self, identities_pure_score: int):
        ...
        identities_normalized_score = (((identities_pure_score / identities_max_score) * NORMALIZATION_MAX_SCORE) * (identities_max_percent / ONE_HUNDRED))
        if NUMBER_OF_DECIMAL_PLACES >= ZERO:
            identities_normalized_score = round(identities_normalized_score, NUMBER_OF_DECIMAL_PLACES)
    ...
    def calculate_histories_normalized_score(self, histories_pure_score: int):
        ...
        histories_normalized_score = (((histories_pure_score / histories_max_score) * NORMALIZATION_MAX_SCORE) * (histories_max_percent / ONE_HUNDRED))
        if NUMBER_OF_DECIMAL_PLACES >= ZERO:
            histories_normalized_score = round(histories_normalized_score, NUMBER_OF_DECIMAL_PLACES)
    ...
    def calculate_volumes_normalized_score(self, volumes_pure_score: int):
        ...
        volumes_normalized_score = (((volumes_pure_score / volumes_max_score) * NORMALIZATION_MAX_SCORE) * (volumes_max_percent / ONE_HUNDRED))
        if NUMBER_OF_DECIMAL_PLACES >= ZERO:
            volumes_normalized_score = round(volumes_normalized_score, NUMBER_OF_DECIMAL_PLACES)
    ...
    def calculate_timeliness_normalized_score(self, timeliness_pure_score: int):
        ...
        timeliness_normalized_score = (((timeliness_pure_score / timeliness_max_score) * NORMALIZATION_MAX_SCORE) * (timeliness_max_percent / ONE_HUNDRED))
        if NUMBER_OF_DECIMAL_PLACES >= ZERO:
            timeliness_normalized_score = round(timeliness_normalized_score, NUMBER_OF_DECIMAL_PLACES)
    ...
    # round down by giving the number of decimal places
    # noinspection PyMethodMayBeStatic
    # def round_down(self, num, decimals=0):
    #     multiplier = 10 ** decimals
    #     return math.floor(num * multiplier) / multiplier

    def resolve_and_save_score_change(self, profile: Profile, rule_code: str, recent_rule_score: int, normalized_revised_rule_score: int):
        ...
        if not initial_score_change_store_config and recent_rule_score is None:
            profile.score += normalized_revised_rule_score
            profile.last_score_change += normalized_revised_rule_score
            profile.last_update_date = datetime.today()
            return normalized_revised_rule_score

        recent_rule_score = ZERO if recent_rule_score is None else recent_rule_score
        if normalized_revised_rule_score == recent_rule_score:
            return ZERO
        rs: ScoreReason = self.ds.get_score_reason_by_rule_code(rule_code)
        score_change = normalized_revised_rule_score - recent_rule_score
