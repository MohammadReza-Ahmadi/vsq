...
public class FileUploaderController {
    ...
    @PutMapping(value = "/ngx-upload/{uniqueId}")
    public ResponseEntity<StringResponse> ngxUpload(@PathVariable String uniqueId, final HttpServletRequest request)
            throws Exception{
        ContentRange contentRange = ServletUtil.getContentRange(contentRangePattern, request);
        FileMetadata fileMetadata = fileUploadService.findByUniqueId(uniqueId);
    ...
    public ResponseEntity<StringResponse> uploadChunk(final HttpServletRequest request,
        ...
        if(exisFileMetadata!=null && FileStatus.COMPLETED.equals(exisFileMetadata.getFileStatus())){
            ...
            metadata = fileUploadService.findByHash(metadata.getHash());
